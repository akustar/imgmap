<template>
  <div class="draggable" :class="{'is-resizable': isResizable, 'is-draggable': isDraggable} " :style="draggableBoxStyling" @mousedown.stop="onMouseDown">
    <slot></slot>
    <div class="resizable" @mousedown="isResizable = true"></div>
    <button type="button" class="button delete" @mousedown.stop @click="$emit('deleteMaps', index)">×</button>
  </div>
</template>
    ...
    methods: {
      ...
      onMouseMove (event) {
        ...
        if (this.isResizable) {
          ...
          let width = Math.min(event.pageX - draggableRect.left + this.scrollXFrame(event) - this.container.scrollLeft + 3, constraintRect.width - left)          
          let height = Math.min(event.pageY - draggableRect.top + this.scrollYFrame(event) - this.container.scrollTop + 3, constraintRect.height - top)
          ...
          if (left + this.width > constraintRect.width) left = constraintRect.width - this.width
          if (top + this.height > constraintRect.height) top = constraintRect.height - this.height
